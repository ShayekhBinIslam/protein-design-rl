...
class DockqPDataset(Dataset):
    """Load dataset from aggregated aalpha submission data ."""

    def __init__(self, cfg, transform=None, split: Optional[str] = None):
        """
        Arguments:
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            "target_sequence": rows["target_sequence"],
            "binder_sequence": rows["binder_sequence"],
            "sequence": rows["sequence"],
            "target_name": rows["target_name"],
            "dockq": rows["dockq"],
            "afm_ptm": rows.get("afm_ptm", -1),
            "afm_iptm": rows.get("afm_iptm", -1),
        }
...
class DockqPV0(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            train_set = DockqPDataset(self._dataset_cfg, transform=self.transforms, split="train")
            test_set = DockqPDataset(self._dataset_cfg, transform=self.transforms, split="test")
            validation = DockqPDataset(self._dataset_cfg, transform=self.transforms, split="validation")
