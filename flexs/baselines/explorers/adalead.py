...
class Adalead(flexs.Explorer):
    ...
    def propose_sequences(self, measured_sequences: pd.DataFrame, round_idx: int):
        ...
        top_fitness = measured_sequences["true_fitness"].max()
        top_inds = measured_sequences["true_fitness"] >= top_fitness * (
            ...
            for i in range(0, len(parents), self.eval_batch_size):
                ...
                while (
                    ...
                    while len(children) < len(nodes):
                        ...
                        child = s_utils.generate_random_mutant(node, self.mu * 1 / len(node), self.alphabet,)
