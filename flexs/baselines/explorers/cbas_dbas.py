...
class CbAS(flexs.Explorer):
    ...
    def __init__(
        ...
        """
        generator = VAE(len(starting_sequence), alphabet=alphabet, epochs=vae_epochs, verbose=False)
        name = f"{algo_type}_Q={Q}_generator={generator.name}"
        super().__init__(
            model,
            name,
            rounds,
    ...
    def propose_sequences(self, measured_sequences_data: pd.DataFrame, round_idx: int):
        ...
        if last_round == 0:
            ...
            rewards = preds
            edit_distances = np.array([int(editdistance.eval(seq, self.starting_sequence)) for seq in sequences])
            return sequences, preds, rewards, edit_distances, None
        ...
        # percentile
        gamma = np.percentile(last_round_sequences["true_fitness"], 100 * self.Q)
        initial_batch = last_round_sequences["sequence"][
            last_round_sequences["true_fitness"] >= gamma
        ].to_numpy()
        ...
        while self.model.cost - previous_model_cost < self.model_queries_per_batch:
            ...
            # cbas and dbas mostly the same except cbas also does an importancesampling step
            if self.algo == "cbas":
