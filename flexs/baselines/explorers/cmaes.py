...
class CMAES(flexs.Explorer):
    ...
    def propose_sequences(self, measured_sequences: pd.DataFrame, round_idx: int):
        ...
        measured_sequence_dict = dict(
            zip(measured_sequences["sequence"], measured_sequences["true_fitness"])
        )

        # Keep track of new sequences generated this round
        top_idx = measured_sequences["true_fitness"].argmax()
        top_seq = measured_sequences["sequence"].to_numpy()[top_idx]
        top_val = measured_sequences["true_fitness"].to_numpy()[top_idx]
