...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        self, obs_dim: int, size: int, batch_size: int = 32, alpha: float = 0.6, priority_weight: float = 0.4
    ):
    ...
    def sample_batch(self, priority_weight: float = 0.4) -> Dict[str, np.ndarray]:
        """Sample a batch of experiences."""
        assert len(self) >= self.batch_size
        assert priority_weight > 0
        ...
        weights = np.array([self._calculate_weight(i, priority_weight) for i in indices])
    ...
    def _calculate_weight(self, idx: int, priority_weight: Optional[float]):
        ...
        max_weight = (p_min * len(self)) ** (-priority_weight)

        # calculate weights
        p_sample = self.sum_tree[idx] / self.sum_tree.sum()
        weight = (p_sample * len(self)) ** (-priority_weight)
